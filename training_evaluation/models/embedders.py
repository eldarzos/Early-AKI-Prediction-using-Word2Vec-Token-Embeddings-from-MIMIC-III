...
class Embedder(nn.Module):
    def __init__(self, n_tokens, latent_dim, dt=1.0, weighted=True):
        ...
        self.embedX = CustomEmbedding(n_tokens, latent_dim, padding_idx=0)
        if self.weighted:
            self.embedW = CustomEmbedding(n_tokens, 1)
...
class CustomEmbedding(nn.Module):
    ...
    def load_pretrained_weights(self, weights_path):
        """Load pre-trained embedding weights from a .npy file."""
        print(f"Loading pre-trained weights from {weights_path}...")
        
        # Load the .npy file using NumPy
        pretrained_weights = np.load(weights_path)
        
        # Convert the NumPy array to a PyTorch tensor
        pretrained_weights_tensor = torch.from_numpy(pretrained_weights).float()

        if pretrained_weights_tensor.shape != self.weights.shape:
            raise ValueError(
                f"Shape mismatch! Model's embedding layer requires {self.weights.shape}, "
                f"but the pre-trained weights file has shape {pretrained_weights_tensor.shape}."
            )
            
        with torch.no_grad():
            self.weights.data.copy_(pretrained_weights_tensor)
            
        print("Successfully loaded pre-trained weights into CustomEmbedding layer.") 